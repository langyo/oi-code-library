...
int main() {
    ...
    for(auto i = in1.crbegin(); i < in1.crend(); ++i) {
        if('0' <= *i && *i <= '9') num1.push_back(static_cast<short>(*i - '0'));
        else num1.push_back(static_cast<short>(*i - 'A') + 10);
    }
    for(auto i = in2.crbegin(); i < in2.crend(); ++i) {
        if('0' <= *i && *i <= '9') num2.push_back(static_cast<short>(*i - '0'));
        else num2.push_back(static_cast<short>(*i - 'A') + 10);
    }
    ...
    for(auto i = sum.crbegin(); i != sum.crend(); ++i) {
        if(B < 10) cout << *i;
            ...
            if((*i) >= 10) cout << static_cast<char>(static_cast<short>('A') + (*i - 10));
            else cout << *i;
        }
    }
    cout << endl;
