...
int main() {
    ...
    auto bfs = [&](int x, int y) {
        ...
        while(!stack.empty()) {
            ...
            // 有效性检查
            if(map.at(pos.first).at(pos.second) == 1) continue;
            if(map.at(pos.first).at(pos.second) == -1) continue;
    ...
    // 从边缘染色
    for(int i = 0; i < n; ++i) bfs(0, i), bfs(i, 0), bfs(n - 1, i), bfs(i, n - 1);

    // 染色闭合部分
    for(auto &i : map) for(auto &j : i) if(j == 0) j = 2;
    // 将染成 -1 的部分回滚为 0
    for(auto &i : map) for(auto &j : i) if(j == -1) j = 0;

    // 输出
    for(auto &i : map) {
        cout << i.front();
        for(auto j = i.cbegin() + 1; j != i.cend(); ++j) cout << " " << *j;
        cout << endl;
    }
