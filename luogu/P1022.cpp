...
int main() {
    ...
    auto evaluator = [&]() {
        if((isPlus && isLeft) || ((!isPlus) && (!isLeft))) {
            // 在左侧且为加法，或在右侧且为减法
            if(isUnknown && hasNum) unknown_weight += num;
            else if(isUnknown && (!hasNum)) __, unknown_weight += 1;
            else constant += num;
    ...
    if(unknown_weight == 0 || constant == 0) result = 0.0;
    else result = - static_cast<double>(constant) / static_cast<double>(unknown_weight);
