...
int main(){
	...
	for(int left = 0; left < 1000; ++left) {
		for(int right = 0; right < 1000; ++right) {
			// 取得各个位的火柴棒个数并加起来
			int n = 0;
			if(left == 0) n += numCount[0];
			else for(int i = left; i != 0; n += numCount[i % 10], i /= 10);
			if(right == 0) n += numCount[0];
			else for(int i = right; i != 0; n += numCount[i % 10], i /= 10);
			if(left + right == 0) n += numCount[0];
			else for(int i = left + right; i != 0; n += numCount[i % 10], i /= 10);
			// 判定是否与所给根数一样
			if(n == k) {
				#ifdef DEBUG
