...
int main(){
    ...
    for(int i = 1; i < 10; ++i) subIntList.push_back(stoi(match[i].str())); // 提取由正则匹配到的前九个数字
    char matchCode = match[10].str()[0]; // 最后一位字符

    int code = 0; // 由机器生成的校验码存储变量
    for(int i = 0; i < 9; ++i) code += subIntList[i] * (i + 1);
    code %= 11;
    char codeToChar = code == 10 ? 'X' : to_string(code)[0];

    if(matchCode == codeToChar) cout << "Right";
    else cout << in.substr(0, 12) << codeToChar;
